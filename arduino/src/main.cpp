...
int yellowDuration = 1000; // 노란불 지속 시간 (기본값: 1초)
int greenDuration = 2000;  // 초록불 지속 시간 (기본값: 2초)

// 버튼 2 인터럽트 핸들러 (긴급 모드)
void handleButton2Press() {
  if (digitalRead(buttonPin2) == LOW) {
    button2Pressed = !button2Pressed; // 버튼 상태 토글
    if (button2Pressed) {
...
void handleButton3Press() {
  if (digitalRead(buttonPin3) == LOW) {
    button3Pressed = !button3Pressed; // 버튼 상태 토글
    if (button3Pressed) {
...
// 메인 루프
void loop() {
  if (!systemOn) return; // 시스템이 OFF 상태면 아무 작업도 하지 않음
